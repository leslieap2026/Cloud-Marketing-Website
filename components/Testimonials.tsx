  ...
  return (
    <section id="results" className="section bg-surface-2">
      <div className="container-x">
        ...
        {/* Results band */}
        <div className="mb-14 grid grid-cols-4 gap-[18px] rounded-card p-10 text-white max-[720px]:grid-cols-2 bg-[linear-gradient(160deg,#2a525e,#1d3a42)]">
          {results.map((r, i) => (
            <Reveal key={r.label} delay={i * 110} className="text-center">
              <span className="gradient-text block text-[2.4rem] font-extrabold">{r.num}</span>
            ...
            <Reveal key={t.name} delay={i * 110} className="h-full">
              <figure className="m-0 h-full rounded-card border border-line bg-surface p-8 shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
                ...
                <figcaption className="flex items-center gap-3 text-[0.9rem] text-muted">
                  <span className="grid h-[42px] w-[42px] shrink-0 place-items-center rounded-full bg-[linear-gradient(135deg,#0cc0df,#264a54)] font-semibold text-white">
                    {t.initials}
                  </span>
                  <span>
